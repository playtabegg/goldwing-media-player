...
The same arithmetic as ``tools/wti_ed25519.py`` at the estate root (which
also signs) and ``rialto2/rialto_updater/_ed25519.py`` (which runs on a
pressed disc). Auditability over speed: affine coordinates, the arithmetic
reads like the RFC, no key generation and no signing because the Player
never holds a private key. ``S`` is range-checked and point decoding
rejects non-canonical encodings and small-order keys.
"""
